...
class EmTransactionStatus implements TransactionStatus {
    protected bool $completed = false;
    protected ?Savepoint $savepoint = null;

    public function __construct(
        protected ?EmTransactionObject $transaction = null,
    ...
    public function createAndHoldSavepoint(): void {
        if (!$this->getTransaction()->isSavepointAllowed()) {
            throw new NestedTransactionNotSupportedException(
                "Cannot create a nested transaction because savepoints are not supported by your Doctrine driver"
            );
        }
